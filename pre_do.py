...
def image_cut(img):
    print(img.shape)
    r_t=960-int(img.shape[1])
    b_t=720-int(img.shape[0])
    dst=cv2.copyMakeBorder(img,top=0,bottom=b_t,left=0,right=r_t,borderType=cv2.BORDER_REPLICATE)
    return dst
def load_gt_boxes(path):
    ...
    for iter_, bb in zip(range(len(bbs)), bbs):
        ...
        ignore = int(bb[10])
        ignore = ignore or (bbtype != 'animal')
